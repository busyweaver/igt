...
#    print 'K%d' % K
    ...
    tmp=trouvernoeuds(N,re[0],int(math.ceil(re[1]/float(kf[re[0]]))),ri[0],ri[1])
    print 'tmp = %s' % (str(tmp))
    maj_arbre(tmp,to)

    trouverarbre(int(math.ceil(re[1]/float(CON[N][re[0]]/float(F[re[0]])))),re[0],F,to)          

def trouvernoeuds(N,k,ab,i,ab2):
    l = composition(N-k,i)
    comp=l[int(math.ceil(ab2/float(binomial(k,i)*pow(2,k-i))))-1]
    print 'comp choisie %s' % (str(comp))
    #rajouter les noeuds 1
    ra= maj_noeuds(comp,N)
    const_arbre(N,i,k,comp)
    return ra



def maj_noeuds(comp,N):
...
def recursive_generator(N):
    ...
    for e in [0]:
        ...
        while nb_ok<=N*N*N:
            ...
            n=random.randint(1,kf[N])
            #n=2173
            print 'arbre %d' % n
            trouverarbre(n,N,kf,elem[ind])
            ajout_res(res[ind],elem[ind])
   #         print 'neouds a la fin %s' % (str(noeuds))
  #          print 'res!! %s' % (str(res[0]))
            nb_ok=nb_ok+1
...
if(sys.argv[2]=='0'):
   ...
   if(int(sys.argv[3])>int(sys.argv[1])):
       ...
       coq=un_arbre(int(sys.argv[3]),int(sys.argv[1]))
       # for e in coq:
       #     print '\n'
       #     for i in e :
       #         if not(i==None):
